...
public abstract class NormalHelper {
	...
	public static int computePackedNormal(ModelQuadView q) {
		...
		return VertexUtil.packNormal(normX, normY, normZ);
	}

	public static int packedNormalFromDirection(Direction direction) {
		Vec3i normal = direction.getNormal();

		return VertexUtil.packNormal(normal.getX(), normal.getY(), normal.getZ());
	}

	public static void unpackNormalTo(int packedNormal, Vector3f normal) {
		normal.set(VertexUtil.unpackN1(packedNormal), VertexUtil.unpackN2(packedNormal), VertexUtil.unpackN3(packedNormal));
	}
}
