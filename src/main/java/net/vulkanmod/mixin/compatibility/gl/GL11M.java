...
public class GL11M {
    ...
    /**
     * @author
     * @reason Route direct GL scissor calls through Vulkan dynamic scissor for HUD and config-screen clipping.
     */
    ...
    public static void glTexParameteri(@NativeType("GLenum") int target, @NativeType("GLenum") int pname, @NativeType("GLint") int param) {
        // Only 2D textures are emulated; other targets must not crash the caller.
    ...
    // Real swapchain readback is not wired yet; return deterministic black data
    // instead of leaking stale caller buffer contents.
    @Overwrite(remap = false)
